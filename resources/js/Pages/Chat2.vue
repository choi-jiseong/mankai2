      ...
      <div class="min-w-full border rounded lg:grid lg:grid-cols-3">
        <div class="border-r border-gray-300 lg:col-span-1">
          ...
          <ul class="overflow-auto h-[32rem]">
            ...
            <li v-for="(room, index) in rooms" :key="room.id">
              <a
                @click="fetchMessages(room.id)"
                class="flex items-center px-3 py-2 text-sm transition duration-150 ease-in-out border-b border-gray-300 cursor-pointer hover:bg-gray-100 focus:outline-none">
                <img class="object-cover w-10 h-10 rounded-full"
                  src="https://cdn.pixabay.com/photo/2018/09/12/12/14/man-3672010__340.jpg" alt="username" />
                <div class="w-full pb-2">
                  <div class="flex justify-between">
                    <span class="block ml-2 font-semibold text-gray-600">{{ ChatUsers[index] }}</span>
        ...
        <div class="hidden lg:col-span-2 lg:block">
          <div class="w-full">
            ...
            <div @contextmenu.prevent @click="hideContextMenu()" @contextmenu="showContextMenu($event, message)">
                <ul id="chatBody" class="space-y-2 flex flex-col-reverse w-full p-6 overflow-y-auto h-[38rem] scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch" >
                    <li class="flex"  v-for="(message, index) in messages.data" :key="index" :class="message.user.id == user.id ? 'justify-end' : 'justify-start'">
                        ...
                        <div :id="'message-'+message.id" class="message relative max-w-xl px-4 py-2 text-gray-700 rounded-lg shadow " :class="message.user.id == user.id ? 'bg-red-300' : 'bg-gray-100'">
                            {{ message.message }}
                        </div>
                    </li>
            ...
            <div v-show="this.currentRoom" class="flex items-center justify-between w-full p-3 border-t border-gray-300">
        ...
        methods : {
            ...
            sendMessage() { //메세지 보내기
                if(this.newMessage == '') {
                    ...
                    // const me =
                    axios.post('/chat/send', {message: this.newMessage, room_id : this.currentRoom}).then(response => {
